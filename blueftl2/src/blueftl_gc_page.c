...
#include "blueftl_mapping.page.h"
...
struct flash_block_t *reserved = (struct flash_block_t *)-1;
...
int32_t gc_page_trigger_gc(struct ftl_context_t *ptr_ftl_context, int32_t bus, int32_t chip)
{
	...
	bus=0;chip=0;
	struct flash_block_t *victim = select_victim(ptr_ssd, bus, chip, 2);
	...
	// victim->nr_valid_pages = 0;
	// victim->nr_invalid_pages = 0;
	// victim->nr_free_pages = ptr_ssd->nr_pages_per_block;
	// victim->last_modified_time = timer_get_timestamp_in_us();
